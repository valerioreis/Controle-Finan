...
.alert{border-radius:12px;padding:11px 13px;margin-bottom:12px;font-size:12px;line-height:1.6;cursor:pointer;}
...
function renderCartTabs(){const tabs=document.getElementById('cart-tabs');const show=['dashboard','lancamentos','relatorios'].includes(page);tabs.style.display=show?'flex':'none';if(!show)return;const opts=[{id:'all',nome:'Geral',emoji:'🌐'},...CARTEIRAS];tabs.innerHTML=opts.map(c=>`<div class="cart-tab ${carteira===c.id?'t-'+(c.id):'t-inactive'}" onclick="setCarteira('${c.id}')">${c.emoji} ${c.id==='all'?'Geral':c.nome.split(' ')[0]}</div>`).join('');}
